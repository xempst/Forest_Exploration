...
{
	int h_pos, h_nor, h_tex, h_tan;
	h_pos = h_nor = h_tex = h_tan = -1;
	...
	if (tanBufID != 0) {
		...
		if (h_tan != -1 && tanBufID != 0) {
			...
			CHECKED_GL_CALL(glVertexAttribPointer(h_tan, 2, GL_FLOAT, GL_FALSE, 0, (const void*)0));
		}
	}
...
void Shape::calTangent() {
	float deltaPos1, deltaPos2;
	float deltaUv1, deltaUv2;

	for (size_t v = 0; v < posBuf.size() / 3; v++) {
		deltaPos1 = posBuf[v * 3 + 1] - posBuf[v * 3];
		deltaPos2 = posBuf[v * 3 + 2] - posBuf[v * 3];

		deltaUv1 = texBuf[v * 3 + 1] - texBuf[v * 3];
		deltaUv2 = texBuf[v * 3 + 2] - texBuf[v * 3];

		//float r = 1/ (deltaUv1.x)
	}

	// send tangent buffer to GPU
