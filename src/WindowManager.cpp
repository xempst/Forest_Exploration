...
bool WindowManager::init(int const width, int const height)
{
	...
	glfwSetCursorPosCallback(windowHandle, mouse_position_callback);
...
void WindowManager::mouse_position_callback(GLFWwindow* window, double posX, double posY) {
	if (instance && instance->callbacks) {
		instance->callbacks->mousePositionCallback(window, posX, posY);
	}
}
